...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "POST" && path.startsWith("/inject/")) {
      ...
      return json(
        { ok: true, channelId, message: "Test follow event injected" },
        200,
      );
    }
      ...
      return json(
        {
          ...
          channelId,
          message:
            "Webhook subscription cleared - will re-subscribe on next /channel/ request",
        },
        200,
      );
    }
...
async function handleChannel(slug: string, env: Env): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch(
      `${env.KICK_API_BASE}/channels?slug=${encodeURIComponent(slug)}`,
      {
        headers: {
    ...
    if (!res.ok) {
      return new Response(`Kick API error: ${res.status}`, {
        status: res.status,
      });
    }
    ...
    const channelId = (channel.broadcaster_user_id ??
      channel.id ??
      channel.user_id) as number | undefined;
