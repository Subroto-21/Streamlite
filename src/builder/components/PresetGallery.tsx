...
import { PRESETS, buildPresetConfig, buildCustomConfig, type OverlayPreset } from '../presets'
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: '1', 'min-width': '180px' }}>
        <span style={{ 'font-size': '13px', 'font-weight': '700', color: 'var(--text-primary)', display: 'block', 'margin-bottom': '2px' }}>Custom</span>
        <span style={{ 'font-size': '11px', color: 'var(--text-tertiary)', 'line-height': '1.4' }}>
          Pick {MIN_CUSTOM_COLORS}–{MAX_CUSTOM_COLORS} colors — we'll build the theme (darkest → background, lightest → text, most vivid → accent)
        </span>
      </div>

      <div style={{ display: 'flex', 'align-items': 'center', gap: '14px', 'flex-wrap': 'wrap', padding: '6px 4px' }}>
        <For each={colors()}>
          {(color, i) => (
            <div style={{ position: 'relative', width: '32px', height: '32px', 'flex-shrink': '0' }}>
              <input
                ...
                style={{
                  width: '32px', height: '32px', border: '2px solid var(--surface-4)',
              ...
              <Show when={colors().length > MIN_CUSTOM_COLORS}>
                <button
                  ...
                  style={{
                    ...
                    'border-radius': 'var(--radius-xs)', border: '1px solid var(--surface-2)', cursor: 'pointer', padding: '0',
                    background: 'var(--danger)', color: '#fff', 'font-size': '9px', 'font-weight': '700',
                    'line-height': '14px', 'text-align': 'center', 'box-shadow': '0 1px 4px rgba(0,0,0,0.6)',
                  }}
                >✕</button>
              </Show>
            </div>
        ...
        <Show when={colors().length < MAX_CUSTOM_COLORS}>
          <button
            ...
            style={{
              ...
              border: '2px dashed var(--border-brand)', cursor: 'pointer',
              background: 'transparent', color: 'var(--green-500)', 'font-size': '16px',
      ...
      <button
        ...
        style={{
          'flex-shrink': '0', padding: '9px 16px', 'border-radius': 'var(--radius-md)',
          border: '1px solid var(--border-brand)', cursor: 'pointer',
