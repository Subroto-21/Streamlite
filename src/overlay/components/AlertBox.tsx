...
import type { AlertEvent, AlertMediaFields, WidgetStyle } from '../../shared/types'
...
function mediaFor(alert: AlertEvent, style: Props['style']): { mediaUrl: string; soundUrl: string } {
  if (alert.type === 'follow') return { mediaUrl: style.followMediaUrl ?? '', soundUrl: style.followSoundUrl ?? '' }
  if (alert.type === 'subscription') return { mediaUrl: style.subMediaUrl ?? '', soundUrl: style.subSoundUrl ?? '' }
  return { mediaUrl: style.giftMediaUrl ?? '', soundUrl: style.giftSoundUrl ?? '' }
}
              ...
              <Show when={mediaFor(alert(), props.style).mediaUrl}>
                {(mediaUrl) => (
                  <Show
                    ...
                    fallback={<img src={mediaUrl()} alt="" style={{ 'max-height': '60%', 'max-width': '100%', 'object-fit': 'contain' }} />}
                  >
                    <video src={mediaUrl()} autoplay muted loop style={{ 'max-height': '60%', 'max-width': '100%', 'object-fit': 'contain' }} />
                  </Show>
                )}
              </Show>
