...
function resolveLabel(type: StreamLabelType): string {
  if (type === "latestFollower") {
    const alert = [...alertStore.alerts]
      .reverse()
      .find((a) => a.type === "follow");
    return alert?.username ?? "-";
  }
  if (type === "latestSub") {
    const alert = [...alertStore.alerts]
      .reverse()
      .find((a) => a.type === "subscription");
    return alert?.username ?? "-";
  }
  if (type === "latestGiftSub") {
    const alert = [...alertStore.alerts]
      .reverse()
      .find((a) => a.type === "gift_sub");
    return alert?.username ?? "-";
  }
  if (type === "topGifter") {
    const top = alertStore.alerts
      ...
      .reduce<
        (typeof alertStore.alerts)[number] | undefined
      >((best, a) => ((a.quantityGifted ?? 0) > (best?.quantityGifted ?? 0) ? a : best), undefined);
    return top ? `${top.username} (${top.quantityGifted ?? 1})` : "-";
  }
  if (type === "followerCount")
    return hasFollowerCount() ? followerCount().toLocaleString() : "-";
  if (type === "subCount")
    return hasSubCount() ? subCount().toLocaleString() : "-";
    ...
    <div style={widgetStyleToCSS(props.style)}>
      ...
      >
        <Show
          ...
          fallback={
            <span
              style={{ opacity: "0.4", "font-size": "0.85em", margin: "auto" }}
            >
              No labels added
            </span>
          }
