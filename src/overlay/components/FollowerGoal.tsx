...
const FollowerGoal: Component<Props> = (props) => {
  const percent = () =>
    Math.min(100, (followerCount() / (props.style.goalTarget || 1)) * 100)
    ...
    <div style={widgetStyleToCSS(props.style)}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', 'justify-content': 'space-between', 'font-size': '0.85em' }}>
          ...
          <Show when={hasFollowerCount()} fallback={<span style={{ opacity: '0.4' }}>—</span>}>
            <span>{followerCount().toLocaleString()} / {props.style.goalTarget.toLocaleString()}</span>
          </Show>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            width: hasFollowerCount() ? `${percent()}%` : '0%',
            height: '100%',
