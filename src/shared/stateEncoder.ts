...
export function decompressState(str: string): LayoutConfig {
  try {
    ...
    console.error(
      "[Streamlite] Failed to decompress state - using default:",
      err,
    );
...
if (import.meta.env.DEV) {
  ...
  const roundTripOk =
    JSON.stringify(roundTripped) === JSON.stringify(DEFAULT_CONFIG);
  ...
  if (!roundTripOk) {
    console.error(
      "[Streamlite] stateEncoder round-trip FAILED - check lz-string version",
    );
  }
}
