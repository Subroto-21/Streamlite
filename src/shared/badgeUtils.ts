...
export const BADGE_META: Record<ChatBadge, { icon: string; label: string }> = {
  ...
  subgifter100: { icon: '/badges/subgifter100.svg', label: 'Sub Gifter (100+)' },
}

export function getBadgeFallback(type: string, text?: string): { imageUrl: string; label: string } {
...
export function getBadgeLabel(name: string, metadata?: Record<string, unknown>, text?: string): string {
...
if (import.meta.env.DEV) {
  ...
  for (const [input, expected] of cases) {
    ...
    if (result !== expected) {
      console.error('[badgeUtils] FAIL derivePrimaryRole', { input, expected, result })
    } else {
      passed++
    }
  }
  if (passed === cases.length) console.log(`[badgeUtils] all ${passed} self-tests passed`)
}
